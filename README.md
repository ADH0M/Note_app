...
model AIAnalytics {
  id           String   @id @default(auto()) @map("_id") @db.ObjectId
  userId       String
  feature      String   // 'suggestion', 'summary', 'plan', 'coach'
...
User (1) ──────< (N) AITaskSuggestion
User (1) ──────< (N) AISummary
User (1) ──────< (N) AIDailyPlan
User (1) ──────< (1) AIConversation
